...
namespace FFF
{
  ...
  std::vector<double> FitWrapper::MCMCFit(const std::vector<double>& initparams, const DataWrapper* data,
      size_t steps, float burnin, float jump)
  {
    TNtuple* lspace =  MCMCMapLikelihood(initparams, data, steps, burnin, jump);
  ...
  TNtuple* FitWrapper::MCMCMapLikelihood(const std::vector<double>& initparams, const DataWrapper* data,
      size_t steps, float burnin, float jump)
  {
    ...
    double lcurrent = GetNLL(params, data);
    ...
    for (size_t istep=0;istep<steps;istep++){
      ...
      double lnew = GetNLL(newParams, data);
  ...
  std::vector<double> FitWrapper::SAnnealFit(const std::vector<double>& initparams, const DataWrapper* data,
      size_t steps, float burnin, float jump)
  {
    unsigned long accepted = 0;
    unsigned long total = 0;
  ...
  std::vector<std::pair<double, double> > FitWrapper::GetFrequentistIntervals(std::vector<std::pair<double, double> > param_limits,
      ...
      std::vector<double> initparams, const DataWrapper* data,
      double cl,
      size_t nfake) {
    ...
    TNtuple* lspace_mc = MCMCMapLikelihood(initparams, data);
    std::vector<double> bestfit = SAnnealFit(initparams, data);
    ...
    for (size_t i=0; i<grid_steps[0]; i++) {
      ...
      for (size_t j=0; j<grid_steps[1]; j++) {
        ...
        // Ld = L(Ns, Ti | data)
        double ld = GetNLL(norms_here,data);
        double rd = ld - ml;

        std::vector<double> rs;
        for (size_t k=0; k<nfake; k++) {
          DataWrapper* griddata = GenerateData(norms_here);

          // Lx = L(Ns, Ti | fake data)
          double lx = GetNLL(norms_here, griddata);

          // Lf = L(hat(Ns), hat(Ti) | fake data)
          std::vector<double> bff = SAnnealFit(norms_here, griddata);
